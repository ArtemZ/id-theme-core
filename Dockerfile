...
RUN ./kcadm.sh config credentials --server http://localhost:8080/auth --realm master --user admin --password password
RUN ./kcadm.sh update realms/master -s sslRequired=NONE
RUN ./kcadm.sh update realms/master -s "loginTheme=core"
RUN ./kcadm.sh update realms/master -s "adminTheme=keycloak"
